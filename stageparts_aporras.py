...
class EndShaftSlider (object):
    ...
    TOL_BEARING_L = kstage.TOL_BEARING_L
    ...
    def __init__ (self, slidrod_r, holdrod_r, holdrod_sep, 
        ...
        if slidrod_r >= 6:
            ...
            BOLT_D = 3 # smaller bolts when the shaft are less 12mm diameter
            BOLTPUL_D = 3
...
#        else: # 'y': default
        ...
        sliderod = fcfun.addCyl_pos (r = slidrod_r + self.ROD_SPACE,
                               h = slid_y +2,
                               name = "sliderod",
                               axis = 'y',
                               h_disp = y_offs - 1)
        ...
        bolt_lowmid_pos_y = ( self.holdrod2end
                            + holdrod_r
                            + .5 * self.OUT_SEP_L
                            + y_offs )
        ...
        # alejandro porras:
        #bolt_pullow_pos_y = (  bolt_lowmid_pos_y
        #                    + self.OUT_SEP_L)

        bolt_pullow_pos_y = self.length/2 + y_offs - 57/2.
        #bolt_pulhigh_pos_y = (  self.length
        #                     - 2.5 * self.OUT_SEP_L
        #                     - 2 * holdrod_r  # no _tol
        #                     + y_offs)
        # alejandro porras:
        #bolt_pulhigh_pos_y = (  bolt_highmid_pos_y
        #                      - self.OUT_SEP_L)
        bolt_pulhigh_pos_y = self.length/2 + y_offs + 57/2.

        # external diameter of the idle pulley. The function is called further
        # down
        idpull_nlist = kcomp.idpull_dict[BOLTPUL_D]
        pulleyextdiam = partgroup.getmaxwashdiam(idpull_nlist)



        # CHECK: there is no space, length too small
        rodsep_extra = (  bolt_pulhigh_pos_y 
                        - bolt_pullow_pos_y
                        - pulleyextdiam)
        if rodsep_extra <0:
            logger.error("Rod separation %d, too small", holdrod_sep)
            logger.error("It is %d shorter", rodsep_extra)
        else:
            logger.debug("Rod separation %d", holdrod_sep)
            logger.debug("extra space %d", rodsep_extra)
...
#       
        ...
        boltpull0 = addBolt (
                            r_shank   = BOLTPUL_SHANK_R_TOL,
                            l_bolt    = 2 * slid_z,
                            r_head    = BOLTPUL_NUT_R_TOL,
                            l_head    = BOLTPUL_NUT_L,
                            hex_head  = 1, extra=1,
                            support = 1, 
        ...
        #                                          -slid_z)
        # alejandro porras
        boltpull0.Placement.Base =  FreeCAD.Vector ( -bolt_left_pos_x,
                                                   bolt_pulhigh_pos_y,
                                            #self.length/2 + y_offs + 57/2.,
                                                  -slid_z)
        ...
        #                                           -slid_z)
        # alejandro porras
        boltpull1.Placement.Base =  FreeCAD.Vector ( -bolt_left_pos_x,
                                                    bolt_pullow_pos_y,
                                            #self.length/2 + y_offs - 57/2.,
                                                   -slid_z)
...
h_yslid_nx = EndShaftSlider(slidrod_r = RODY_R,
                                        holdrod_r = RODY_R,
...
class CentralSlider (object):
    ...
    def __init__ (self, rod_r, rod_sep, name, belt_sep,
        ...
        self.length = ( rod_sep
                    + 2 *(bearing_r + 2 * self.SEP + 2 * BOLT_NUT_R_TOL))
        ...
        if dlg_y != 0:
            ...
            # linear guide 
            tot_lg_y_w = lg_y_w +  2 * BOLT_NUT_R_TOL + 4 * self.SEP
        if dlg_ny !=0:
            lg_ny_w = dlg_ny['rail']['rw']
            tot_lg_ny_w = lg_ny_w +  2 * BOLT_NUT_R_TOL + 4 * self.SEP
...
#        cutlist.append (csboltpull1)
        ...
        boltend_posx = slid_x/2. - self.SEP - BOLT_NUT_R_TOL
        boltend_posy = slid_y/2. - self.SEP - BOLT_NUT_R_TOL
        ...
        # The 1st iteration: sg= -1, lg= dlg_nx
        # The 2nd iteration: sg=  1, lg= dlg_x
...
# Microscope slides (portas) dimensions (see kstage.py)
...
# ---------- class CentralSliderHole ----------------------------------------
# Creates the central slider that moves on the X axis, with a center hole 
# The slider runs on 2 pairs of linear bearings.
# The slider is referenced on its center
# Creates both sides, the upper part and the lower part, it also creates the
# linear bearings
# Arguments:
...
class CentralSliderHole (object):
    ...
    def __init__ (self, rod_r, rod_sep, name, belt_sep,
        ...
        h_lmuu_x_y = comps.LinBearing (
                         r_ext = bearing_r,
                         r_int = rod_r,
                         h     = bearing_l,
                         name  = "lm" + str(int(2*rod_r)) + "uu_cen_x_y",
                         axis  = 'x',
                         h_disp = - bearing_l/2.0,
                         r_tol  = self.MLTOL,
                         h_tol  = self.TOL_BEARING_L)

        bearing_pos_x = self.MIN_BEAR_SEP + BOLT_HEAD_R + bearing_l/2.
        h_lmuu_x_y.BasePlace ((bearing_pos_x, rod_sep / 2., 0))
        cutlist.append (h_lmuu_x_y.bearing_cont)

        # Cloning the other 3 linear bearings
        h_lmuu_nx_y = comps.LinBearingClone (
                                      h_lmuu_x_y, 
                                      "lm" + str(int(2*rod_r)) + "uu_cen_nx_y",
                                      namadd = 0)
        h_lmuu_nx_y.BasePlace ((-bearing_pos_x, rod_sep / 2., 0))
        cutlist.append (h_lmuu_nx_y.bearing_cont)

        h_lmuu_x_ny = comps.LinBearingClone (
                                      h_lmuu_x_y, 
                                      "lm" + str(int(2*rod_r)) + "uu_cen_x_ny",
                                      namadd = 0)
        h_lmuu_x_ny.BasePlace ((bearing_pos_x, -rod_sep/2., 0))
        cutlist.append (h_lmuu_x_ny.bearing_cont)

        h_lmuu_nx_ny = comps.LinBearingClone (
                                      h_lmuu_x_y, 
                                      "lm" + str(int(2*rod_r)) + "uu_cen_nx_ny",
                                      namadd = 0)
        h_lmuu_nx_ny.BasePlace ((-bearing_pos_x, -rod_sep/2., 0))
        cutlist.append (h_lmuu_nx_ny.bearing_cont)
...
# --------------------- class PortaTrayHole -------------
# Tray for the portas. The place where the portas are is hollow
        
# Microscope slides (portas) dimensions (see kstage.py)
# porta_l : length of the porta
# porta_w : width of the porta
# porta_h : heigth of the porta
# n_porta : number of portas
# porta_sep : separation between portas,
# sep_ext: separation from a porta to the end. Exterior separation
# tray_h: height of the tray, where the portas will be
# toplimit: 1: one of the w-ends of each porta will have a 45degree limit
#           in that case, the hole for the porta will have twice as heigth
#      ______ ........
#           / ........+ extra height
#      ____|__________+ porta_h
#
# extraporta_l: 1: the length hole of the porta will be larger, to have more
#               space to introduce the porta, 
# axis_portas: 'x' or 'y'. The axis where the portas will be set
#      it will be done axis_portas = 'x', and if y it will be rotated
# clamp:0: no clamp
#       1: clamp on the positive side (if axis_portas='x', clamps will be on y
#      -1: clamp on the negative side (if axis_portas='x', clamps will be on -y
#      it will be done axis_portas = 'x' and clamp + 1 (on y+)
#         it will be rotated if any other configuration
# handhole: 1: it will make a hole to make easier to get the porta
# fco: 0: it doesn't create the freecad object,
#      1: it creates the freecad object with the name
# name: name of the freecad object
# pos_z: position of the base on axis Z

# Attributes:
# all the arguments and:
# baseplace_fco: FreeCAD.Vector with the position of the freecad object,
#           if self.fco == 1
# baseplace_shp: FreeCAD.Vector with the position of the shape.
#         if the fco has been created, and baseplace_shp is changed, the 
#         fco doesn't change its position. But if a fco is created afterwards
#         with the shape that has changed its position, the new fco will be
#         in that new position, having V0 as its position
# tray_l: length of the base where the portas are
# tray_w: width of the base where the portas are
# shp: the shape of the tray
# shp_holes: the shape of the fusion of all the holes
# shp_clamp_list: a list with shapes of the clamps
# fco: the FreeCAD Object. If 0 means it has not been created
# fco_clamp_group: the FreeCAD Object of the compound of the clamps fco.
#            If 0 means it has not been created
...
class PortaTrayHole (object):
    """
    Creates a tray for the portas that has a hole below the portas,
    so it is hollow under the portas
    For the microscope slides (portas) dimensions see kstage.py
    ...
    Args:
        ...
        porta_sep : separation between portas,
        sep_ext: separation from a porta to the end. Exterior separation
                 In the case that there are bolts (see argument bolts)
                 sep_ext will be modified if it is not large enough to include
                 the bolt holes
        tray_h: height of the tray, where the portas will be
        bolts : Metric Diameter (3 for M3, 4 for M4, ...) of the bolts on the
                corners (and on the center if there are many portas)
                If 0 there will be no bolts
        toplimit: 1: one of the w-ends of each porta will have a 45degree limit
                 in that case, the hole for the porta will have twice as heigth
         ______ ........
              / ........+ extra height
        ...
        extraporta_l: 1: the length hole of the porta will be larger, to have
                  more space to introduce the porta, 
        axis_portas: 'x' or 'y'. The axis where the portas will be set
                 it will be done axis_portas = 'x', and if y it will be rotated
        clamp: 0: no clamp
               1: clamp on the positive side
                  vg. if axis_portas='x', clamps will be on y
              -1: clamp on the negative side 
                  vg. if axis_portas='x', clamps will be on -y
               Initially, the tray will be positioned having axis_portas = 'x'
               and clamp + 1 (on y+). It will be rotated if any other
    ...
    Attributes:
        ...
        fco_clamp_group: the FreeCAD Object of the compound of the clamps fco.
               If 0 means it has not been created
        bolt_pos_list: list of FreeCAD.Vector that indicates where the bolts
               are. Referenced to the bottom part (where the shank is, not the
               head of the bolt).
    ...
    def __init__ (self, porta_l, porta_w, porta_h, n_porta, porta_sep,
                  sep_ext,
                  tray_h,
                  bolts = 3,
                  toplimit = 0, extraporta_l = 0,
                  axis_portas = 'x',
                  clamp = 0, handhole = '0',
                  fco=0, name='portatray', pos_z = 0):

        doc = FreeCAD.ActiveDocument

        self.baseplace_fco = V0
        self.baseplace_shp = V0
        self.porta_l     = porta_l
        self.porta_w     = porta_w
        self.porta_h     = porta_h
        self.n_porta     = n_porta
        self.porta_sep   = porta_sep
        #self.sep_ext     = sep_ext # value given below
        self.tray_h      = tray_h
        self.toplimit    = toplimit
        self.extraporta_l = extraporta_l
        self.clamp       = clamp
        self.axis_portas = axis_portas
        self.pos_z       = pos_z
        # probably nutbolt_d already has tolerances, so just in case, convert
        # to integer (metric) and add tol
        #nutbolt_d_metric = int(nutbolt_d) 
        ...
        if bolts > 0:
            # check if the separation to the end is long enough
            # has_nut = 0 because it only is going to be the head, because
            #   the nut will be in the other piece
            bolt_sep = fcfun.get_bolt_end_sep (bolt_d= bolts, hasnut=0)
            if sep_ext < 2 * bolt_sep:
                sep_ext = 2 * bolt_sep
        self.sep_ext = sep_ext

        # calculate the rotation:
        if axis_portas == 'x':
            if clamp == -1:
                rot = FreeCAD.Rotation(VZ, 180)
            else: # No rotation
                rot = FreeCAD.Rotation(VZ, 0)
        elif axis_portas == 'y':
            if clamp == -1:
                rot = FreeCAD.Rotation(VZ, 90)
            else: # if 0 it doesnt matter 90 or -90
                rot = FreeCAD.Rotation(VZ, -90)
        else:
            logger.error("Invalid value for axis_portas: %s", axis_portas)

        # In case we want to use a specific tolerance
        tol_porta_l = TOL # 0.6
        tol_porta_w = 0.5

        porta_l_tol = porta_l + tol_porta_l
        porta_w_tol = porta_w + tol_porta_w
        if toplimit == 0:
            portahole_h = porta_h + TOL
        else:
            portahole_h = 2 * porta_h 


        # setback to support the porta
        porta_sback = 2.5
        porta_l_sup = porta_l - 2* porta_sback
        porta_w_sup = porta_w - 2* porta_sback
        # distance between the center of 2 portas
        portacenter_dist = porta_w + porta_sep            

        if clamp != 0: # there is a clamp
            clamprail_w = 10.
            clamprail_ws = 6.
            # depth (legnth) of the clamp (extrusion)
            clamp_d = clamprail_ws
            # heigth of the clamp rail
            clamprail_h = 4.
            # depth (length) of the clamp rail (extrusion)
            clamprail_d = 2 * clamp_d + porta_sback + extraporta_l
            # part of the clamp above the rail
            clampover_h = portahole_h + porta_h 
            # the bolt that will be to atach the clamp and hold a spring
            clampbolt_d = 3.
            # put 2mm below the hole, and calculate the relative position
            clampboltpos_z = clampbolt_d/2. + 2.
            clampboltrelpos_z = clampboltpos_z / clamprail_h
            # extra width due to the clamp
            # +3 to support the bolt
            minclampspace =  (2 * clamp_d) + 3 
            if sep_ext < minclampspace :
                clampextratray_w =  minclampspace - sep_ext
            else:
                clampextratray_w = 0
            # a list with the shapes of the clamps
            shp_clamp_list = []
        else:
            clamprail_w = 0
            clamprail_ws = 0
        ...
        #print str(clampextratray_w)
        self.clamprail_w = clamprail_w
        self.clamprail_ws = clamprail_ws
        self.clamp_d = clamp_d
        self.clamprail_h = clamprail_h
        self.clampover_h = clampover_h
        self.clampbolt_d = clampbolt_d
        self.clampboltpos_z = clampboltpos_z
        self.clampboltrelpos_z = clampboltrelpos_z
        self.clampextratray_w = clampextratray_w
        self.shp_clamp_list = shp_clamp_list


        tray_l = (n_porta * porta_w) + (n_porta -1) * porta_sep + 2*sep_ext
        self.tray_l = tray_l
        # add the extraporta_l and 2 * clamprail_ws
        tray_w = porta_l + 2 * sep_ext + extraporta_l + clampextratray_w
        self.tray_w = tray_w
        ...
        #                Y 
        #                :
        #        ________:________ ............. tray_w/2 + traybox_pos_y
        #       |        :        |     :
        #       |        :        |     :
        #       |        :        |     + extraporta_l + clampextratay_w
        #       |        :        |     :
        #       |  _____ : _____  |.....:
        #       | |     |:|     | |
        #       | |     |:|     | |...... traybox_posy (center of shp_tray_box)
        #       | |     |:|     | |
        #       | |     |:|.....|.|.....................X
        #       | |     | |     | |
        #       | |     | |     | |
        #       | |_____| |_____| |
        #       |_________________|............. -tray_w/2 + traybox_pos_y
        #
        fillrad = 2.

        # since one side of the width may be larger asymetically, we put
        # the center of the porta in the center
        traybox_pos_y = (extraporta_l + clampextratray_w)/2.

        # to have in the center of the porta in x0 y0. 
        # extraporta is outside
        # check if pos should be changed, or it complicate it too much to have
        # it
        shptraybox_pos = FreeCAD.Vector(0, traybox_pos_y, pos_z)

        shp_tray_box = fcfun.shp_boxcenfill (
                                   x= tray_l,
                                   y= tray_w,
                                   z= tray_h,
                                   fillrad = fillrad,
        ...
        if bolts > 0:
            bolt_pos_x = tray_l/2. - bolt_sep
            bolt_pos_y = tray_w/2. - bolt_sep + traybox_pos_y
            bolt_pos_ny = -(tray_w/2. - bolt_sep) + traybox_pos_y
            bolt_pos_list = []
            self.bolt_pos_list = bolt_pos_list
            for i_pos_x in (-bolt_pos_x, bolt_pos_x):
                for i_pos_y in (bolt_pos_ny, bolt_pos_y):
                    bolt_pos = FreeCAD.Vector(i_pos_x, i_pos_y, pos_z)
                    # in the list, the position is the lowest, where the shank
                    # is, to continue in the other piece
                    bolt_pos_list.append(bolt_pos)
                    # the bolt position is referenced to the head: add tray_h
                    bolt_pos.z = pos_z + tray_h
                    shp_bolt = fcfun.shp_bolt(
                                r_shank= kcomp.D912[bolts]['shank_r_tol'],
                                l_bolt = tray_h,
                                r_head = kcomp.D912[bolts]['head_r_tol'],
                                l_head = kcomp.D912[bolts]['head_l'] + TOL,
                                hex_head = 0,
                                support = 0,
                                axis= '-z',
                                pos = bolt_pos)
                    trayhole_list.append(shp_bolt)

        # Making the holes for the portas
        # PORTA_SUP_IN is the same in the 4 directions
        #
        #        Y
        #        |_X
        #
        ...
        #    |  |____________|..................:             :
        #    |  : :        : :                                :
        #    |__:_:________:_:________________________________:
        #    :  : :        : : 
        #    :  : :...+....: : 
        #    :  : porta_w_sup:
        #    :  :            :
        ...
        #    |  |____________|.....+ portahole_h   #   2*porta_h if toplimit==1
        #    |    |        |                       #     porta_h if toplimit==0
        #    |____|________|_______
        #
        #


        # Hole. All the way down, interior
        # Position of the center of the first porta, depending on the
        # number of portas
        #
        #  When 2 portas:
        #   porta_w (no tolerances when making calculations)
        #  ..+..
        #  :   : porta_sep/2.
        #  :   :+.
        #  :___: :  ___
        ...
        #    :       :
        #    :.......:
        #        + porta_w + porta_sep == portacenter_dist
        # 
        #  Therefore the initial position will be:
        #  (porta_w + porta_sep) * (n_porta - 1)/2
        #  portacenter_dist * (n_porta - 1)/2
        #  and add ind * portacenter_dist to the following portas
        
        portahole_pos_z = pos_z + tray_h-portahole_h 
       

        if handhole == 1:
            #make a hole to take the portas easily
            tray_l = (n_porta * porta_w) + (n_porta -1) * porta_sep + 2*sep_ext
            handhole_l = tray_l - 2*sep_ext + 10
            # how long it goes under the bottom of the porta
            handhole_depth = 3.
            handhole_h = 2*(portahole_h+handhole_depth+1)
            handhole_pos_z = portahole_pos_z-handhole_depth
            if sep_ext < 5:
                logger.error ("sep_ext %d, too small", sep_ext)
            shp_handhole = fcfun.shp_boxcenfill(
                       x = handhole_l,
                       # change the value if you want longer or shorter
                       y = 0.8 * porta_l,
                       z = 2*(portahole_h+handhole_depth+1),
                       fillrad = handhole_depth,
                       fx=1, fy=1, fz=1,
                       cx=1, cy=1, cz=0,
                       pos = FreeCAD.Vector(0,0,portahole_pos_z-handhole_depth))

            #handhole_face = fcfun.shp_rndrect_wire (
            #                             x=handhole_l+10, 
            #                             y=porta_l/3.,
            #                             r=5,
            #                             pos_z= portahole_pos_z-handhole_depth)
            #shp_handhole = handhole_face.extrude(FreeCAD.Vector(0,0,
            #                                     portahole_h+handhole_depth+1))
            #Part.show(shp_handhole)
            trayhole_list.append(shp_handhole)

        # position of the first porta
        portacenter_pos_x = - 0.5 * (n_porta -1) * portacenter_dist 

        for ind in xrange(n_porta):
            portathruhole_pos = FreeCAD.Vector(portacenter_pos_x,
                                               0, pos_z-1) # -1 to cut
            
            # complete hole that goes all the way (smaller XY)
            shp_portathruhole = fcfun.shp_boxcenfill (
                                        x = porta_w_sup,
                                        y = porta_l_sup,
                                        z = tray_h + 2, #2: to cut
                                        fillrad = 1,
                                        fz=1,
                                        cx = 1, cy=1, cz=0,
                                        pos = portathruhole_pos)

            # hole of the size of the porta (larger XY)
            portahole_pos = FreeCAD.Vector(portacenter_pos_x,
                                           0,
                                           portahole_pos_z) 

            # extra z is porta_h. So if we chamfer it, the radius
            # will be 2*porta_h
            # in case toplimit == 1
            #         ________
            #________| /......porta_h
            ...
            #
            #
            shp_portahole = fcfun.shp_boxcenxtr(
                                        x = porta_w_tol,
                                        y = porta_l_tol,
                                        z = portahole_h, 
                                        cx = 1, cy=1, cz=0,
                                        ...
                                        pos = portahole_pos)
            if toplimit == 1:
                chmf_zpos = portahole_pos.z + portahole_h + porta_h
                shp_portahole = fcfun.shp_filletchamfer (
                                       shp = shp_portahole,
                                       e_len = porta_w_tol,
                                       fillet = 0, # chamfer
                                       # STOL: there are smaller portas 0.9
                                       radius = 2* porta_h + kcomp.STOL,
                                       axis = 'x',
                                       xpos_chk = 0,
                                       ypos_chk = 1,
                                       zpos_chk = 1,
            ...
            shp_trayholes = shp_portathruhole.fuse(shp_portahole)

            if clamp != 0:
                shp_faceclamp = fcfun.shp_face_rail(
                                           rail_w = clamprail_w,
                                           rail_ws = clamprail_ws,
                                           rail_h = clamprail_h,
                                           rail_h_plus = clampover_h,
                                           axis_l = 'y',
                                           axis_b = '-z',
                                           hole_d = clampbolt_d,
                                           hole_relpos_z = clampboltrelpos_z)
                faceclamp_pos_y = porta_l_tol/2
                faceclamp_pos_z = portahole_pos.z - clamprail_h
                shp_faceclamp.Placement.Base = FreeCAD.Vector(portacenter_pos_x,
                                                              faceclamp_pos_y,
                                                              faceclamp_pos_z)
                shp_baseclamp = fcfun.shp_extrud_face(shp_faceclamp,
                                        clamp_d, 
                                        VY)
                # clamp rail hole:
                shp_faceclamp_hole = fcfun.shp_face_rail(
                                           rail_w = clamprail_w,
                                           rail_ws = clamprail_ws,
                                           rail_h = clamprail_h,
                                           offs_w = TOL/2.,
                                           ...
                                           hole_d = 0,
                                           hole_relpos_z = clampboltrelpos_z)
                faceclamp_hole_pos_y =  porta_l_tol /2 - porta_sback - 1
                shp_faceclamp_hole.Placement.Base = FreeCAD.Vector(
                                                        portacenter_pos_x,
                                                        faceclamp_hole_pos_y,
                                                        faceclamp_pos_z)
                shp_clamp_hole = fcfun.shp_extrud_face(shp_faceclamp_hole,
                                                       # +1 to cut
                                                       clamprail_d +1, 
                                                       VY)
                #Part.show(shp_clamp_hole)
                # Make a hole for the bolt. It has to be loose: + TOL
                clampbolthole_pos_y = faceclamp_hole_pos_y + clamprail_d
                clampbolthole_pos_z = faceclamp_pos_z + clampboltpos_z
                clampbolthole_pos = FreeCAD.Vector(portacenter_pos_x,
                                                   clampbolthole_pos_y,
                                                   clampbolthole_pos_z)
                shp_clampbolthole = fcfun.shp_cyl(
                                        r=(clampbolt_d+TOL)/2.,
                                        h = sep_ext + clampextratray_w + 2,
                                        normal = VY,
                                        pos = clampbolthole_pos)
                #Part.show(shp_clampbolthole)
                                                  
                shp_trayholes = shp_trayholes.multiFuse([shp_clamp_hole,
                                                         shp_clampbolthole])
                # clamp: add a top limit for the porta
                topclamp_h = portahole_h + kcomp.STOL
                topclamp_pos_y = faceclamp_pos_y - portahole_h
                topclamp_pos_z = portahole_pos.z + clampover_h - topclamp_h
                topclamp_pos = FreeCAD.Vector(portacenter_pos_x,
                                              topclamp_pos_y,
                                              topclamp_pos_z)
                shp_topclamp_sq = fcfun.shp_boxcen(clamprail_ws,
                                                   portahole_h + 1,
                                                   topclamp_h,
                                           # centered on x, if is on axis x
                                                   cx = 1, 
                                                   cy = 0, 
                                                   cz = 0,
                                                   pos = topclamp_pos)
                shp_topclamp = fcfun.shp_filletchamfer (
                                       shp = shp_topclamp_sq,
                                       e_len = clamprail_ws,
                                       fillet = 0, # chamfer
                                       # radius cannot be same size as edge
                                       radius = topclamp_h- kcomp.STOL,
                                       axis = 'x',
                                       xpos_chk = 0,
                                       ypos_chk = 1,
                                       zpos_chk = 1,
                                       ...
                                       zpos = topclamp_pos_z)
                doc.recompute()
                shp_clamp = shp_baseclamp.fuse(shp_topclamp)

                # clamp: remove on top to make an indent
                # just 1 mm indent, 2 over to cut and make the chamfer
                clampind_h = 1
                clampind_pos_y = faceclamp_pos_y + 1
                clampind_pos_z = portahole_pos.z + clampover_h - clampind_h
                clampind_pos = FreeCAD.Vector(portacenter_pos_x,
                                              clampind_pos_y,
                                              clampind_pos_z)
                clampind_x = 2 * clamprail_ws 
                clampind_y = clamprail_ws - 2
                shp_clampind = fcfun.shp_boxcenchmf(clampind_x,
                                                    clampind_y,
                                                    clampind_h + 2,
                                                    clampind_h,
                                                    fx=1, fy=0, fz=0,
                ...
                #Part.show(shp_clampind)
                doc.recompute()
                shp_clamp = shp_clamp.cut(shp_clampind)
                shp_clamp.Placement.Rotation = rot

                shp_clamp = shp_clamp.removeSplitter()
                doc.recompute()
                                                   
                shp_clamp_list.append(shp_clamp)
                #Part.show(shp_clamp)

            #Part.show(shp_trayholes)

            # position of the next porta
            portacenter_pos_x += portacenter_dist 
            # don't append the last one to perform the multiFuse of shapes
            if (ind != n_porta-1):
                trayhole_list.append(shp_trayholes)

        shp_holes = shp_trayholes.multiFuse(trayhole_list)

                                            
        shp_tray_box.Placement.Rotation = rot
        shp_holes.Placement.Rotation = rot

        self.shp_holes = shp_holes

        shp_tray = shp_tray_box.cut(shp_holes)
        doc.recompute()
        self.shp = shp_tray
        if fco == 1:
            fco_tray = doc.addObject("Part::Feature", name)
            fco_tray.Shape = shp_tray
            self.fco = fco_tray
            if clamp != 0:
                fco_clamp_list = []
                for ind, clamp in enumerate (shp_clamp_list):
                    fco_clamp = doc.addObject("Part::Feature",
                                               name + "clamp_" + str(ind))
                    fco_clamp.Shape = clamp
                    fco_clamp_list.append(fco_clamp)
                fco_clamp_group = doc.addObject("Part::Compound",
                                                name + "_clamp")
                fco_clamp_group.Links = fco_clamp_list
                self.fco_clamp_group = fco_clamp_group

    # METHODS

    # gets the freecad object, if it has not been created, it will create one
    def get_fco (self):
        if self.fco is None:
            doc = FreeCAD.ActiveDocument
            fco_tray = doc.addObject("Part::Feature", self.name)
            fco_tray.Shape = self.shp
            self.fco = fco_tray
            if self.clamp != 0:
                fco_clamp_list = []
                for ind, clamp in enumerate (self.shp_clamp_list):
                    fco_clamp = doc.addObject("Part::Feature",
                                               name + "clamp_" + str(ind))
                    fco_clamp.Shape = clamp
                    fco_clamp_list.append(fco_clamp)
    ...
    # check what to move, the shape or the freecad.Vector
    # Moving the FreeCAD object, doesn't move the shape
    def baseplace_fco (self, position = V0):
        if self.fco is None:
            logger.error("No FreeCAD object, create it with get_fco")
        elif type(position) is tuple:
            self.base_place = position
            self.fco.Placement.Base = FreeCAD.Vector(position)
            self.fco_clamp_group.Placement.Base = FreeCAD.Vector(position)
        elif type(position) is FreeCAD.Vector:
            self.base_place = (position.x, position.y, position.z)
            self.fco.Placement.Base = position
            self.fco.clamp_group.Placement.Base = position

    # Moving the shape, doesn't move the FreeCAD Object (fco)
    # but if a new fco is created, it will have the new position of the shape
    def baseplace_shp (self, position = V0):
        if type(position) is tuple:
            self.baseplace_shp = position
        ...
        self.shp.Placement.Base = vpos
        if self.clamp != 0:
            for clamp_i in self.clamp_list:
                clamp_i.Placement.Base = vpos



#doc = FreeCAD.newDocument()
#h_portatrayhole = PortaTrayHole(
...
## ------------------ Base of the portas ----------------------------
#
#portabase = addBox (x = kstage.PORTABASE_W,
#                    y = kstage.PORTABASE_L,
#                    z = kstage.PORTABASE_H,
#                    name = "portabase",
#                    cx = 1, cy= 0)
#
#portabase_pos_y = (   kstage.CIT_Y
#                    - (1.5 * kstage.ALU_W)
...
#
#portahole_list = []
#for i in xrange(kstage.N_PORTA):
#    portahole_i = addBox (x = kstage.PORTA_L + TOL,
#                        y = kstage.PORTA_W + TOL,
#                        z = kstage.PORTA_H + 2,  # +2 to cut
#                        name = "portahole_" + str(i),
#                        cx = 1, cy= 0)
#    portahole_pos_y =  (i+1)*kstage.PORTA_SEP + i *kstage.PORTA_W - TOL/2.0
...
#doc = FreeCAD.newDocument()
#CentralSlider (rod_r = kstage.ROD_R, rod_sep = 150.0, name="central_slider")
